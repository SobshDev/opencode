...
You can also list configured models without starting the TUI:

```bash frame="none"
opencode models
```

These terminal interfaces remain separate from the model-facing [`models` tool](/docs/tools#models), which lets a model discover exact references before delegating work with [`model_call`](/docs/tools#model_call).
